...
<div class="mb-4">
    ...
    <input type="text" name="nama" id="nama" value="{{ old('nama', $pembina->nama ?? '') }}"
...
<div class="mb-4">
    ...
    <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan', $pembina->jabatan ?? '') }}"
        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
    @error('jabatan')
        <span class="text-red-500 text-xs">{{ $message }}</span>
    @enderror
...
<div class="mb-4">
    ...
    <div class="mt-2 flex items-center gap-x-4">
        @php
            ...
            $foto_url =
                isset($pembina) && $pembina->foto
                    ? asset('storage/' . $pembina->foto)
                    : asset('images/default-profile.png');
        @endphp
        <img id="foto-preview" src="{{ $foto_url }}" alt="Preview"
            class="h-24 w-24 object-cover rounded-full bg-gray-100">
