@csrf <div class="mb-4">
    ...
    <input type="text" name="nama" id="nama" value="{{ old('nama', $anggota->nama ?? '') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
    @error('nama')
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
    <div>
        ...
        <select name="tingkat_kelas" id="tingkat_kelas" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
            <option value="" disabled {{ old('tingkat_kelas', $anggota->tingkat_kelas ?? '') == '' ? 'selected' : '' }}>Please select</option>
    ...
    <div>
        ...
        <select name="pengelompokan_kelas" id="pengelompokan_kelas" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
             <option value="" disabled {{ old('pengelompokan_kelas', $anggota->pengelompokan_kelas ?? '') == '' ? 'selected' : '' }}>Please select</option>
             @for ($i = 1; $i <= 12; $i++)
...
<div class="mb-4">
    ...
    <input type="text" name="kelas" id="kelas" value="{{ old('kelas', $anggota->kelas ?? '') }}" class="mt-1 block w-full bg-gray-100 border-gray-300 rounded-md shadow-sm" readonly>
    @error('kelas')
        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
    @enderror
</div>
