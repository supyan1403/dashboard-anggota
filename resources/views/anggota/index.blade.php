<x-app-layout>
    ...
    <div class="mt-4 flex flex-wrap gap-2">
        <a href="{{ route('anggota.migrate.form') }}" class="inline-flex items-center px-4 py-2 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-600 active:bg-yellow-700 focus:outline-none focus:border-yellow-700 focus:ring ring-yellow-300 disabled:opacity-25 transition ease-in-out duration-150">
            Naik Kelas
        </a>
        <a href="{{ route('anggota.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-800 focus:outline-none focus:border-blue-800 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
            + Tambah Anggota
        </a>
    </div>
</div>
    ...
    <div>
        ...
        <p class="text-4xl font-bold text-gray-900 mt-2">{{ $stats['aktif'] }} <span class="text-2xl font-medium text-gray-500">/ {{ $stats['total'] }} Anggota Aktif</span></p>
    </div>
 ...
 <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
    ...
    <form action="{{ route('anggota.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('anggota.index') }}" class="px-4 py-2 bg-gray-300 text-gray-700 text-sm rounded-lg hover:bg-gray-400">Reset</a>
        </div>
    </form>

    <form action="{{ route('anggota.index') }}" method="GET" class="w-full md:w-1/3">
         <div class="relative">
            <input type="text" name="search" placeholder="Cari Nama atau Kelas..." class="border rounded-lg px-4 py-2 w-full text-sm" value="{{ request('search') }}">
    ...
    <tr class="text-gray-500 uppercase text-sm">
        ...
        <th class="py-3 px-4">
            ...
            @endphp
            <a href="{{ route('anggota.index', array_merge(request()->query(), ['sort' => 'id', 'direction' => $idDirection])) }}" class="flex items-center gap-2">
                <span>ID</span>
                @if (request('sort') == 'id')
                    <i class="fa-solid {{ $idDirection == 'asc' ? 'fa-sort-down' : 'fa-sort-up' }}"></i>
                @else
        ...
        <th class="py-3 px-4">
             @php
                $namaDirection = (request('sort') == 'nama' && request('direction') == 'asc') ? 'desc' : 'asc';
            @endphp
            <a href="{{ route('anggota.index', array_merge(request()->query(), ['sort' => 'nama', 'direction' => $namaDirection])) }}" class="flex items-center gap-2">
                <span>Nama</span>
                @if (request('sort') == 'nama')
                    <i class="fa-solid {{ $namaDirection == 'asc' ? 'fa-sort-down' : 'fa-sort-up' }}"></i>
                @else
                    <i class="fa-solid fa-sort text-gray-300"></i>
                @endif
            </a>
        ...
        <th class="py-3 px-4">
            @php
                $kelasDirection = (request('sort') == 'kelas' && request('direction') == 'asc') ? 'desc' : 'asc';
            @endphp
            <a href="{{ route('anggota.index', array_merge(request()->query(), ['sort' => 'kelas', 'direction' => $kelasDirection])) }}" class="flex items-center gap-2">
                <span>Kelas</span>
                 @if (request('sort') == 'kelas')
                    <i class="fa-solid {{ $kelasDirection == 'asc' ? 'fa-sort-down' : 'fa-sort-up' }}"></i>
                @else
                    <i class="fa-solid fa-sort text-gray-300"></i>
                @endif
            </a>
        ...
        <th class="py-3 px-4">
             @php
                $statusDirection = (request('sort') == 'status' && request('direction') == 'asc') ? 'desc' : 'asc';
            @endphp
            <a href="{{ route('anggota.index', array_merge(request()->query(), ['sort' => 'status', 'direction' => $statusDirection])) }}" class="flex items-center gap-2">
                <span>Status</span>
                @if (request('sort') == 'status')
                    <i class="fa-solid {{ $statusDirection == 'asc' ? 'fa-sort-down' : 'fa-sort-up' }}"></i>
                @else
                    <i class="fa-solid fa-sort text-gray-300"></i>
                @endif
            </a>
