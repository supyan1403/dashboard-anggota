    ...
    <div class="flex flex-col justify-between flex-grow">
        ...
        <div class="flex-1 flex flex-col overflow-hidden ml-64">
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-8">
                @if (session('success'))
                    <div id="alert-session" class="flex items-center p-4 mb-4 text-green-800 rounded-lg bg-green-50 transition-all duration-500 ease-in-out" role="alert">
                        ...
                        <button type="button" onclick="document.getElementById('alert-session').style.display = 'none';" class="ms-auto -mx-1.5 -my-1.5 bg-green-50 text-green-500 rounded-lg focus:ring-2 focus:ring-green-400 p-1.5 hover:bg-green-200 inline-flex items-center justify-center h-8 w-8" aria-label="Close">
    ...
    <script>
        ...
        document.addEventListener('submit', function (e) {
            if (e.target.classList.contains('delete-form')) {
                ...
                const form = e.target;
                Swal.fire({
                    title: 'Konfirmasi Penghapusan', text: "Anda akan menghapus data ini secara permanen.", showCancelButton: true, confirmButtonText: 'Lanjutkan Hapus', cancelButtonText: 'Batal', buttonsStyling: false, customClass: { confirmButton: 'px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 mr-2', cancelButton: 'px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300' }
                }).then((result) => {
